...
TEST_F(RxJobTests, shouldSendOneSegmentUrlMessage)
{
    ...
    auto ack1 = createAck(0, MSG_ID, 0);

    EXPECT_CALL(endPoint, send(SafeMatcherCast<const BufferView&>(IsBufferEq(BufferView(ack1))), ipPort));
...
TEST_F(RxJobTests, shouldSendManySegmentUrlMessage)
{
    constexpr uint16_t MSG_ID = 42u;
    constexpr uint16_t MSG_SIZE = 621u;
    auto toSend = createUrlMessage(MSG_SIZE, 100);
    ...
    // EXPECT_CALL(endPoint, send(SafeMatcherCast<const BufferView&>(IsBufferEq(BufferView(ack1))), ipPort));
    // EXPECT_CALL(rxBufferManager, enqueue_rvalHack(ipPort, IsBufferEq()));
